...
#include <cstdlib>
static GLuint gl_blp_load_data(const BLPFile &file)
{
...
static GLuint gl_blp_load_file(const char *const filename)
{
	FILE *fp;
	size_t fsize;
	void *buffer;
...
void OpenGlResources::load_blp(const char *const blp_filename)
{
	blp_texture_id = gl_blp_load_file(blp_filename);
}

void OpenGlResources::load_blp_data(void *const blp_data)
{
	BLPFile blp(blp_data);
	blp_texture_id = gl_blp_load_data(blp);
}
